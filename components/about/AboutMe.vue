<template>
    ...
    <div class="container relative flex flex-col px-6 mx-auto space-y-8">
      <div
        class="absolute inset-0 z-0 w-2 h-full shadow-md bg-secondary-light dark:bg-secondary-dark left-17 md:mx-auto md:right-0 md:left-0"
      >
        <span></span>
      </div>
      <div
      ...
      >
        ...
        <div class="timeline-container" :class="isLeftContainer(experience.id)">
          ...
          ></div>
          <div
            class="p-6 rounded-md shadow-md bg-neutral-100 dark:bg-secondary-dark"
          >
            <span
              class="font-bold tracking-wide text-ternary-dark dark:text-ternary-light text-md"
              >{{ experience.startDate }}</span
            >
            <h1
              class="pt-1 text-2xl font-bold text-secondary-dark dark:text-primary-light"
            >
              {{ experience.title }}
            </h1>
            <h2
              class="pt-1 text-lg font-bold text-primary-dark dark:text-primary-light"
            >
              {{ experience.subtitle }}
            </h2>
            <p
              v-if="experience.detail.length === 1"
              class="pt-1 text-left text-primary-dark dark:text-primary-light xl:text-center"
            >
              {{ experience.detail[0] }}
            </p>
            <template v-else>
              <li
                ...
                :key="index"
                class="pt-1 text-left list-none text-primary-dark dark:text-primary-light"
              >
                {{ d }}
              </li>
            </template>
  ...
  <style lang="css" scoped>
    ...
    .timeline-pointer {
      @apply absolute inset-0 left-10 h-6 w-4 transform rotate-45
         bg-neutral-100
